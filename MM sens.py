...
for singleMaturity in matVector:
    mm1 = mm.MertonProbabilityOfDefaultWithValueToDebt(2, 0.05, 0.3, singleMaturity)
...
for singleRatio in ratioVector:
    mm2 = mm.MertonProbabilityOfDefaultWithValueToDebt(singleRatio, 0.05, 0.3, 1)
...
for singleDrift in driftVector:
    mm3 = mm.MertonProbabilityOfDefaultWithValueToDebt(2, singleDrift, 0.3, 1)
...
for singleVolatility in volVector:
    mm4 = mm.MertonProbabilityOfDefaultWithValueToDebt(2, 0.05, singleVolatility, 1)
...
plt.xlabel('Maturity')
plt.ylabel('PD (%)')
plt.title('Figure 2.1', fontsize=10)

plt.subplot(2, 2, 2)
plt.scatter(x = ratioVector, y = PDAccordingVD, color = 'red')
plt.xlabel('Value/Debt Ratio')
plt.ylabel('PD (%)')
plt.title('Figure 2.2', fontsize=10)

plt.subplot(2, 2, 3)
plt.scatter(x = driftVector, y = PDAccordingDrift, color = 'black')
plt.xlabel('Drift')
plt.ylabel('PD (%)')
plt.title('Figure 2.3', fontsize=10)

plt.subplot(2, 2, 4)
plt.scatter(x = volVector, y = PDAccordingVolatility, color = 'green')
plt.xlabel('Volatility')
plt.ylabel('PD (%)')
plt.title('Figure 2.4', fontsize=10)

#plt.show()

plt.savefig(r"C:\Users\39328\OneDrive\Desktop\Davide\UNIMI Statale\Tesi di merda\Immagini & Grafici\Merton Sensitivity", dpi = 1500)